...
function openQuoteBuilderOwn() {
    try {
        ...
        if (typeof updateMaterialPreview === 'function') {
            ...
            if (typeof updateCuttingPreview === 'function') updateCuttingPreview('qbOwnCuttingPreview', '');
...
function initQuoteBuilderOwn() {
    ...
    if (cuttingSelect) {
        cuttingSelect.addEventListener('change', (e) => {
            if (typeof updateCuttingPreview === 'function') {
                updateCuttingPreview('qbOwnCuttingPreview', e.target.value);
            }
        });
    }
...
function updateQuoteStepOwn() {
    ...
    const lang = (typeof currentLang !== 'undefined') ? currentLang : 'en';
    const stepLabel = (typeof i18nTranslations !== 'undefined' && i18nTranslations[lang]?.qb_step_label) || 'Step';
    const stepOf = (typeof i18nTranslations !== 'undefined' && i18nTranslations[lang]?.qb_step_of) || 'of';
    ...
    if (currentQuoteStepOwn === 3) {
        ...
        if (typeof updateCuttingPreview === 'function') {
            updateCuttingPreview('qbOwnCuttingPreview', document.getElementById('qbOwnCutting').value);
        }
...
function nextStepOwn() {
    ...
    if (currentQuoteStepOwn === 7) {
        quoteSelectionsOwn.nameset = document.querySelector('input[name="addOwnNameset"]:checked').value;
    }
...
document.getElementById('sendWhatsAppOwnBtn').addEventListener('click', () => {
    ...
    const greeting = (typeof i18nTranslations !== 'undefined' && i18nTranslations[lang]?.wa_greeting) || "Hi ThirtyOne Lab! I'm interested in ordering:";
    const closing = (typeof i18nTranslations !== 'undefined' && i18nTranslations[lang]?.wa_closing) || "Could I get a quotation for this order?";
